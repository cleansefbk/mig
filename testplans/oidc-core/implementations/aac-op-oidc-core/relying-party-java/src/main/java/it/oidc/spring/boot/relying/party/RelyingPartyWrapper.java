...
import it.oidc.handler.RelyingPartyHandler;
import it.oidc.spring.boot.relying.party.config.ClientConfig;
...
public class RelyingPartyWrapper {
	...
	public String revoke(RelyingPartySession session)
			throws OIDCException {
		return relyingPartyHandler.revoke(session);
	}

	public String introspect(RelyingPartySession session)
			throws OIDCException {
		return relyingPartyHandler.introspect(session);
	}
