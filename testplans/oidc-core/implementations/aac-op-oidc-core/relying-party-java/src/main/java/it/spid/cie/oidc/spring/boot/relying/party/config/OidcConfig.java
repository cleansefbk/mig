...
public class OidcConfig extends BaseConfig {
	...
	public List<ProviderInfo> getAacProviders() {
		return aacProviders;
	}
	...
	public Map<String, String> getIdentityProviders(OIDCProfile profile) {
		...
		else if (OIDCProfile.AAC.equals(profile)) {
			for (ProviderInfo provider : aacProviders) {
				result.put(provider.getSubject(), provider.getTrustAnchor());
			}
		}
		return result;
	...
	private List<ProviderInfo> aacProviders = new ArrayList<>();
